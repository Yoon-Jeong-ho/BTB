...
## 왜 이 단위를 배우는가
분산 학습을 처음 보면 `GPU가 여러 개`라는 사실보다 먼저 **같은 코드가 여러 프로세스에서 동시에 실행된다**는 점이 낯설다. `torchrun`은 이 여러 프로세스가 몇 개인지, 각 프로세스의 rank가 무엇인지, 같은 장비 안 local rank가 무엇인지 알려 주는 실행 계약을 만든다. 이 단위는 실제 멀티프로세스를 요구하지 않고, CPU-safe simulation으로 `world size / rank / local rank / gradient averaging` 감각을 먼저 잡게 한다.
