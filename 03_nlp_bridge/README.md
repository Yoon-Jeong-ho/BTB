...
이 구간은 `02_deep_learning`에서 `04_nlp`로 넘어가기 전에 필요한 개념 다리다. 표형 데이터에서는 feature를 사람이 직접 설계했지만, NLP에서는 **문장을 토큰 id로 바꾸고 그 id를 embedding 공간으로 옮긴 뒤, 토큰끼리 attention으로 서로를 참고하게 만드는 과정**이 먼저 필요하다. 이 bridge는 그 전환을 한국어 예제로 천천히 연결한다.
...
- `04_nlp`에 들어가기 전에 “문장이 모델 안에서 어떤 숫자 흐름으로 바뀌는가”를 설명할 수 있게 만든다.
...
이 bridge를 끝내면 `04_nlp`의 텍스트 분류, NER, MRC에서 보게 될 tokenizer / attention mask / embedding layer / transformer encoder block을 더 이상 추상 용어로 보지 않게 된다.
