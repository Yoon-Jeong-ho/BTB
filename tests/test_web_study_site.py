...
class WebStudySiteContractTest(unittest.TestCase):
    ...
    def test_site_has_playwright_qa_and_code_reading_guidance(self) -> None:
        ...
        for token in [
            ...
            "analysisCoreCodeSteps",
            "extractPythonSymbolNames",
            "extractFunctionExcerpt",
            "extractPatternExcerpt",
            "핵심 코드 먼저 보기",
            "Gradient 실습은 이 네 덩어리만 먼저 읽으면 됩니다",
            "Normalization/Regularization 실습은 이 네 부분이 핵심입니다",
            "z-score normalization으로 입력 스케일 맞추기",
            "data loss와 L2 regularization loss 분리하기",
            "gradient에 weight decay 항을 더하기",
            "LayerNorm·Dropout·Weight Decay",
            "data_loss_before_step은 같을 수 있음",
            "regularized_objective_before_step",
            "post_step_data_loss와 weight_norm_after_step",
            "regularizationAnalysisCoreGuide",
            "Regularization 해석 코드는 지표를 결론으로 바꾸는 흐름입니다",
            "필수 metrics가 있는지 먼저 확인하기",
            "weight decay가 같아 보이는 이유를 분리해서 해석하기",
            "stable analysis와 이번 실행 리포트를 나눠 저장하기",
            "해석 리포트 저장",
            "unitCoreCategoryFor",
            "unitCoreTermsFor",
            "단원 핵심 개념",
            ...
            "모델 코드: 후보 모델을 비교 가능하게 만들기",
            "실험 흐름 코드: 준비·학습·평가 연결하기",
            "formatStaticServerDetail",
            "mlDatasetCoreGuide",
            "mlModelsCoreGuide",
            "baseline→선형 모델→트리 모델→MLP 후보",
