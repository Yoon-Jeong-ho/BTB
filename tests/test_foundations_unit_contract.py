...
class TestFoundationsUnitContract(unittest.TestCase):
    ...
    def test_activation_unit_labs_and_analysis_generate_expected_outputs(self) -> None:
        ...
        self.assertGreaterEqual(scratch['figure_sample_count'], 100)
        ...
        self.assertEqual(1000.0, scratch['numeric_stability_demo']['stable_bce_from_logit'])
        self.assertIn('failed', scratch['numeric_stability_demo']['naive_bce_after_sigmoid'])
        self.assertIn('log(0)', scratch['numeric_stability_demo']['why'])

        self.assertEqual([2, 3], framework['class_logits_shape'])
        self.assertIn('activation_reading_guide', framework)
        self.assertIn('loss_reading_guide', framework)
        self.assertEqual(10, len(framework['activation_rows']))
        self.assertEqual({'input', 'relu', 'sigmoid', 'tanh'}, set(framework['activation_rows'][0]))
        self.assertIn('sigmoid', framework['activation_summary'])
        self.assertEqual(0.0, framework['activation_rows'][0]['relu'])
        self.assertLess(framework['activation_rows'][0]['sigmoid'], 0.2)
        self.assertLess(framework['activation_rows'][0]['tanh'], -0.9)
        ...
        self.assertEqual(1000.0, framework['numeric_stability_demo']['stable_bce_with_logits'])
        self.assertIn('포화', framework['numeric_stability_demo']['why'])

        self.assertIn('<svg', figure_text)
        self.assertIn('Activation curves (scratch)', figure_text)
        self.assertIn('dense 161-point curve', figure_text)
        self.assertIn('bounded activation zoom', figure_text)
        self.assertIn('sigmoid(0)=0.5', figure_text)
        ...
        self.assertIn('numeric_stability_demo', observed_text)
        self.assertIn('log(0)', observed_text)
        self.assertIn('latest_report.md', analysis_text)
        self.assertIn('반복 실행 시 불필요한 diff', analysis_text)
        self.assertIn('극단적인 logit', analysis_text)
