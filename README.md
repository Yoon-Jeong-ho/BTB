...
4. 웹에서 전체 커리큘럼과 브라우저별 진행률을 보려면 저장소 루트에서 `python -m http.server 8000`을 실행하고 `http://localhost:8000` 또는 `http://localhost:8000/web/`을 연다. Python 코드 아래 실행 버튼까지 쓰려면 정적 서버 대신 `python scripts/study_server.py --port 8000 --device auto`로 열면 된다. conda 환경은 `--conda-env btb`, CPU 고정은 `--device cpu`, 특정 GPU는 `--device cuda --gpu-index 0`을 붙인다. 이 웹사이트는 문서 파일을 새 탭으로 직접 여는 방식이 아니라 “단원 안내/핵심 이론/준비 확인/실습 코드”를 사이트 안에서 렌더링해 보여주며, 학습 경로 선택·다음 단원 추천·실행 관찰 카드·산출물 뷰어·선택 함수 미리보기·미니 퀴즈·오답노트·자가 점검을 함께 제공한다. 자세한 내용은 `web/README.md`를 따른다. 화면 QA가 필요하면 Playwright 설치 후 `npm run qa:web`을 실행한다.
