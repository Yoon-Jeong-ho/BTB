...
4. 웹에서 전체 커리큘럼과 사용자별 로컬 진행률을 보려면 저장소 루트에서 `python -m http.server 8000`을 실행하고 `http://localhost:8000` 또는 `http://localhost:8000/web/`을 연다. Python 코드 탭의 실행 버튼까지 쓰려면 정적 서버 대신 `python scripts/study_server.py --port 8000 --device auto`로 열면 된다. conda 환경은 `--conda-env btb`, CPU 고정은 `--device cpu`, 특정 GPU는 `--device cuda --gpu-index 0`을 붙인다. 이 웹사이트는 README 파일을 새 탭으로 직접 여는 방식이 아니라 README/THEORY/PREREQS/실습 코드를 사이트 안에서 렌더링해 보여준다. 자세한 내용은 `web/README.md`를 따른다. 화면 QA가 필요하면 Playwright 설치 후 `npm run qa:web`을 실행한다.
